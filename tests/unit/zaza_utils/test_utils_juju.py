...
class TestJujuUtils(ut_utils.BaseTestCase):
    def setUp(self):
        ...
        self.unit0_mock.get_public_address = make_get_public_address("10.0.0.11")
        ...
        self.unit1_mock.get_public_address = make_get_public_address("10.0.0.1")
        ...
        self.subordinate_application_data = {"subordinate-to": [self.application]}
        ...
        self.model.get_units.return_value = [self.unit0_mock, self.unit1_mock, self.unit2_mock]

        # Clouds
        self.cloud_name = "FakeCloudName"
        self.cloud_type = "FakeCloudType"
        self.clouds = {"clouds": {self.cloud_name: {"type": self.cloud_type}}}

        # Controller
        self.patch_object(juju_utils, "controller")
        self.controller.get_cloud.return_value = self.cloud_name

    def _get_application_status(self, application=None, unit=None, model_name=None):
    ...
    def test_get_application_status(self):
        ...
        self.assertEqual(
            juju_utils.get_application_status(application=self.application), self.application_data
        )

        # Unit no application dictionary return
        self.assertEqual(juju_utils.get_application_status(unit=self.unit1), self.unit1_data)
    ...
    def test_get_cloud_configs(self):
        ...
        self.assertEqual(
            juju_utils.get_cloud_configs(self.cloud_name), self.clouds["clouds"][self.cloud_name]
        )
    ...
    def test_get_machines_for_application(self):
        ...
        # Machine data
        self.assertEqual(
            list(juju_utils.get_machines_for_application(self.application)), [self.machine1]
        )

        self.assertEqual(
            list(juju_utils.get_machines_for_application(self.subordinate_application)),
            [self.machine1],
        )

    def test_get_unit_name_from_host_name_maas(self):
        self.assertEqual(juju_utils.get_unit_name_from_host_name("juju-model-1", "app"), "app/1")
        self.machine2_mock.display_name = "node-jaeger.maas"
        self.assertEqual(
            juju_utils.get_unit_name_from_host_name("node-jaeger.maas", "app"), "app/2"
        )
        self.machine0_mock.display_name = "node-bob.maas"
        self.assertEqual(juju_utils.get_unit_name_from_host_name("node-bob.maas", "app"), "app/0")
    ...
    def test_get_unit_name_from_host_name(self):
        ...
        self.assertEqual(
            juju_utils.get_unit_name_from_host_name("juju-model-1", self.application), "app/1"
        )
        self.assertEqual(
            juju_utils.get_unit_name_from_host_name(
                "juju-model-1.project.serverstack", self.application
            ),
            "app/1",
        )

    def test_get_unit_name_from_host_name_bad_app(self):
        self.assertIsNone(juju_utils.get_unit_name_from_host_name("juju-model-12", "madeup-app"))

    def test_get_unit_name_from_host_name_subordinate(self):
        self.patch_object(juju_utils, "get_application_status")
        self.get_application_status.side_effect = self._get_application_status
        self.assertEqual(
            juju_utils.get_unit_name_from_host_name("juju-model-1", self.subordinate_application),
    ...
    def test_get_unit_name_from_ip_address(self):
        ...
        unit_mock4.data = {
            "public-address": "10.0.0.240",
            "private-address": "10.0.0.241",
            "name": "myapp/5",
        }
        unit_mock4.entity_id = "myapp/5"
        self.model.get_units.return_value = [unit_mock3, unit_mock4]
        self.assertEqual(juju_utils.get_unit_name_from_ip_address("10.0.0.12", "myapp"), "myapp/2")
        self.assertEqual(
            juju_utils.get_unit_name_from_ip_address("10.0.0.241", "myapp"), "myapp/5"
        )
    ...
    def test_get_machine_status(self):
        ...
        # All machine data
        self.assertEqual(juju_utils.get_machine_status(self.machine1), {self.key: self.key_data})
        self.get_full_juju_status.assert_called_once()
    ...
    def test_get_machine_uuids_for_application(self):
        ...
        self.assertEqual(
            list(juju_utils.get_machine_uuids_for_application(self.application)), [self.key_data]
        )
    ...
    def test_remote_run(self):
        ...
        # Success
        self.assertEqual(juju_utils.remote_run(self.unit1, _cmd), self.run_output["Stdout"])
        ...
        self.assertEqual(
            juju_utils.remote_run(self.unit1, _cmd, fatal=False), self.error_run_output["Stderr"]
        )
    ...
    def test_get_unit_names(self):
        self.patch(
            "cou.zaza_utils.model.get_first_unit_name",
    ...
    def test_get_relation_from_unit(self):
        ...
        juju_utils.get_relation_from_unit("aunit/0", "otherunit/0", "arelation")
        self.model.run_on_unit.assert_called_with(
            "aunit/0", 'relation-get --format=yaml -r "42" - "otherunit/0"', model_name=None
        )
    ...
    def test_leader_get(self):
        ...
        self.patch_object(juju_utils, "model")
        data = {"foo": "bar"}
        self.model.run_on_leader.return_value = {"Code": 0, "Stdout": str(data)}
    ...
    def test_leader_get_key(self):
        ...
        self.patch_object(juju_utils, "model")
        data = {"foo": "bar"}
        self.model.run_on_leader.return_value = {"Code": 0, "Stdout": data["foo"]}
    ...
    def test_get_machine_series(self):
        self.patch(
            "cou.zaza_utils.juju.get_machine_status",
        ...
        self._get_machine_status.assert_called_with(machine="6", key="series", model_name=None)
        ...
        self.assertEqual(
            sorted(
                juju_utils.get_subordinate_units(
                    ["nova-compute/0", "cinder/1"], status=juju_status
                )
            ),
            sorted(["neutron-openvswitch/2", "cinder-hacluster/0", "cinder-ceph/3"]),
        )
        self.assertEqual(
            juju_utils.get_subordinate_units(
                ["nova-compute/0", "cinder/1"], charm_name="ceph", status=juju_status
            ),
            ["cinder-ceph/3"],
        )
    ...
    def test_get_subordinate_units_lp1987332(self):
        ...
        juju_status.applications = {
            "nova-compute": {
                ...
                "units": {
                    "nova-compute/0": {"subordinates": {"neutron-openvswitch/2": {"charm": ""}}}
                },
            },
            "cinder": {
        ...
        self.model.get_status.return_Value = juju_status
        with mock.patch.dict(os.environ, {"TEST_ZAZA_BUG_LP1987332": "1"}):
            self.assertEqual(
                sorted(
                    juju_utils.get_subordinate_units(
                        ["nova-compute/0", "cinder/1"], status=juju_status
    ...
    def test_get_application_ip(self):
        ...
        self.model.get_application_config.return_value = {"vip": {"value": "10.0.0.10"}}
        self.model.get_units.return_value = [self.unit1_mock]
        self.assertEqual(juju_utils.get_application_ip("app"), "10.0.0.10")
        self.model.get_application_config.return_value = {}
        self.model.get_unit_public_address.return_value = "10.0.0.1"
        self.assertEqual(juju_utils.get_application_ip("app"), "10.0.0.1")
        ...
        self.assertEqual(juju_utils.get_application_ip("app"), "10.0.0.20")

        def _get_k8s_ingress_ip(application, model_name=None):
            raise KeyError

        self.get_k8s_ingress_ip.side_effect = _get_k8s_ingress_ip
        self.assertEqual(juju_utils.get_application_ip("app"), "")
    ...
    def test_is_k8s_deployment(self):
        ...
        self.assertFalse(juju_utils.is_k8s_deployment())
        self.model.get_model_info.return_value = {"provider-type": "kubernetes"}
    ...
    def test_get_k8s_ingress_ip(self):
        ...
        self.model.get_status.return_value = {"applications": {"cinder": cinder_app}}
        self.assertEqual(juju_utils.get_k8s_ingress_ip("cinder"), "10.152.183.31")
