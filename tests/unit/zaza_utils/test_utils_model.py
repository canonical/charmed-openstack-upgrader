...
import cou.zaza_utils as zaza
import cou.zaza_utils.model as model
...
class TestModel(ut_utils.BaseTestCase):
    ...
    def test_set_juju_model_aliases(self):
        ...
        self.assertEqual(model.MODEL_ALIASES, {"alias1": "model1", "alias2": "model2"})

    def test_unset_juju_model_aliases(self):
        model.unset_juju_model_aliases()
        self.assertEqual(model.MODEL_ALIASES, {})
        model.set_juju_model_aliases({"alias1": "model1", "alias2": "model2"})
        model.unset_juju_model_aliases()
        self.assertEqual(model.MODEL_ALIASES, {})
    ...
    def test_get_juju_model_aliases(self):
        model.set_juju_model_aliases({"alias1": "model1", "alias2": "model2"})
        self.assertEqual(model.get_juju_model_aliases(), {"alias1": "model1", "alias2": "model2"})
    ...
    def test_ensure_model_connected_exception(self):
        ...
        with mock.patch.object(zaza, "RUN_LIBJUJU_IN_THREAD", new=False):
            model.sync_wrapper(_wrapper)()
            self.mymodel.disconnect.assert_called_once_with()
            self.mymodel.connect_model.assert_called_once_with(self.mymodel.info.name)

    def _mocks_for_block_until_auto_reconnect_model(self, is_connected=True, is_open=True):
    ...
    def test_block_until_auto_reconnect_model(self):
        ...
        self._wrapper_block_until_auto_reconnect_model(lambda: True)
        with mock.patch.object(zaza, "RUN_LIBJUJU_IN_THREAD", new=False):
    ...
    def test_block_until_auto_reconnect_model_disconnected_sync(self):
        ...
        self.Model_mock.connect_model.assert_has_calls([mock.call("testmodel")])

    def test_block_until_auto_reconnect_model_disconnected_async(self):
        self._mocks_for_block_until_auto_reconnect_model([False, True, True], True)

        async def _async_true():
            return True

        self._wrapper_block_until_auto_reconnect_model(aconditions=[_async_true])
        with mock.patch.object(zaza, "RUN_LIBJUJU_IN_THREAD", new=False):
            model.sync_wrapper(self._wrapper)()
        self.Model_mock.disconnect.assert_has_calls([mock.call()])
        self.Model_mock.connect_model.assert_has_calls([mock.call("testmodel")])
    ...
    def test_block_until_auto_reconnect_model_blocks_till_true(self):
        ...
        self._wrapper_block_until_auto_reconnect_model(condition, wait_period=1.5)
        with mock.patch.object(zaza, "RUN_LIBJUJU_IN_THREAD", new=False):
            model.sync_wrapper(self._wrapper)()
    ...
    def test_get_units(self):
        ...
        self.patch_object(model, "Model")
        self.Model.return_value = self.Model_mock
        self.assertEqual(model.get_units("app"), self.units)

    def test_get_machines(self):
        self.patch_object(model, "get_juju_model", return_value="mname")
        self.patch_object(model, "Model")
        self.Model.return_value = self.Model_mock
        self.assertEqual(model.get_machines("app"), [self.machine3, self.machine7])

    def test_get_first_unit_name(self):
        self.patch_object(model, "get_juju_model", return_value="mname")
        self.patch_object(model, "get_units")
        self.get_units.return_value = self.units
        self.assertEqual(model.get_first_unit_name("model", "app"), "app/2")
    ...
    def test_get_lead_unit(self):
        ...
        self.Model.return_value = self.Model_mock
        # unit2 is the leader
        self.assertEqual(model.get_lead_unit("app", "model"), self.unit2)
    ...
    def test_get_unit_public_address(self):
        ...
        self.patch_object(model, "async_get_unit_public_address__fallback", name="mock_fallback")
        self.patch_object(model, "async_get_unit_public_address__libjuju", name="mock_libjuju")
        ...
        with mock.patch.dict(model.os.environ, _env):
            self.assertEqual(
                model.get_unit_public_address(self.unit1, model_name="a-model"), "fip"
            )
            self.mock_fallback.assert_called_once_with(self.unit1, model_name="a-model")
        ...
        with mock.patch.dict(model.os.environ, _env):
            ...
            self.mock_libjuju.assert_called_once_with(self.unit1, model_name=None)
    ...
    def test_get_unit_public_address__libjuju(self):
        ...
        self.assertEqual(
            sync_get_unit_public_address__libjuju(self.unit1, model_name="a-model"), "ip1"
        )
    ...
    def test_get_unit_public_address__fallback(self):
        ...
        status = yaml.safe_dump(
            {"applications": {"an-app": {"units": {"an-app/0": {"public-address": "2.3.4.5"}}}}}
        )

        async def mock_check_output(*args, **kwargs):
            return {"Stdout": status}

        self.patch_object(model.generic_utils, "check_output", name="async_check_output")
        ...
        self.assertEqual(
            sync_get_unit_public_address__fallback(mock_unit, model_name="b-model"), "2.3.4.5"
        )
        self.async_check_output.assert_called_once_with(
            "juju status --format=yaml -m b-model".split(), log_stderr=False, log_stdout=False
        )
        mock_p_name = mock.PropertyMock(return_value="an-app/1")
        type(mock_unit).name = mock_p_name

        self.async_check_output.reset_mock()
        self.assertEqual(sync_get_unit_public_address__fallback(mock_unit), None)
        self.async_check_output.assert_called_once_with(
            "juju status --format=yaml -m a-model".split(), log_stderr=False, log_stdout=False
        )
    ...
    def test_get_lead_unit_ip(self):
        ...
        self.patch_object(model, "async_get_lead_unit", side_effect=mock_async_get_lead_unit)
        self.patch_object(
            model, "async_get_unit_public_address", side_effect=mock_async_get_unit_public_address
        )
        self.assertEqual(model.get_lead_unit_ip("app", "model"), "ip2")
    ...
    def test_get_unit_from_name(self):
        ...
        self.Model.return_value = self.Model_mock
        # Normal case
        self.assertEqual(model.get_unit_from_name("app/4", model_name="mname"), self.unit2)

        # Normal case with Model()
        self.assertEqual(model.get_unit_from_name("app/4", self.mymodel), self.unit2)

        # Normal case, using default
        self.assertEqual(model.get_unit_from_name("app/4"), self.unit2)
    ...
    def test_get_app_ips(self):
        ...
        self.patch_object(model, "async_get_units", side_effect=mock_async_get_units)
        self.patch_object(
            model, "async_get_unit_public_address", side_effect=mock_async_get_unit_public_address
        )

        self.assertEqual(model.get_app_ips("app", "model"), ["ip1", "ip2"])
        self.async_get_units.assert_called_once_with("app", model_name="model")
        self.async_get_unit_public_address.assert_has_calls(
            [mock.call(self.unit1, model_name="model"), mock.call(self.unit2, model_name="model")]
        )
    ...
    def test_run_on_unit_lc_keys(self):
        self.patch_object(model, "get_juju_model", return_value="mname")
        self.action.data["results"] = {"Code": "0", "stdout": "RESULT", "stderr": "some error"}
        expected = {
            "Code": "0",
            "Stderr": "some error",
            "Stdout": "RESULT",
    ...
    def test_remove_relation(self):
        ...
        self.mymodel.applications["app"].destroy_relation.assert_called_once_with(
            "shared-db", "mysql-shared-db"
        )
    ...
    def test_add_unit(self):
        ...
        self.mymodel.applications["app"].add_unit.assert_called_once_with(count=2, to="lxd/0")
    ...
    def test_add_unit_wait(self):
        ...
        self.mymodel.applications["app"].add_unit.assert_called_once_with(count=2, to="lxd/0")
        self.async_block_until_unit_count.assert_called_once_with("app", 4, model_name=None)
    ...
    def test_destroy_unit(self):
        ...
        self.mymodel.applications["app"].destroy_unit.assert_called_once_with("app/2")
    ...
    def test_destroy_unit_wait(self):
        ...
        self.mymodel.applications["app"].destroy_unit.assert_called_once_with("app/2")
        self.async_block_until_unit_count.assert_called_once_with("app", 1, model_name=None)
    ...
    def test_scale_out(self):
        ...
        self.mymodel.applications["app"].scale.assert_called_once_with(scale=3, scale_change=None)
    ...
    def test_scale_wait(self):
        ...
        self.mymodel.applications["app"].scale.assert_called_once_with(scale=3, scale_change=None)
        self.async_block_until_unit_count.assert_called_once_with("app", 3, model_name=None)
    ...
    def test_scale_back_wait(self):
        ...
        self.mymodel.applications["app"].scale.assert_called_once_with(scale=None, scale_change=-1)
        self.async_block_until_unit_count.assert_called_once_with("app", 1, model_name=None)
    ...
    def test_get_relation_id_interface(self):
        ...
        self.Model.return_value = self.Model_mock
        self.assertEqual(
            model.get_relation_id("app", "app", remote_interface_name="interface"), 51
        )
    ...
    def test_run_action(self):
        ...
        self.Model.return_value = self.Model_mock
        model.run_action("app/2", "backup", action_params={"backup_dir": "/dev/null"})
        self.unit1.run_action.assert_called_once_with("backup", backup_dir="/dev/null")
    ...
    def test_run_action_on_leader(self):
        ...
        self.Model_mock.get_action_output = _fake_get_action_output
        self.Model.return_value = self.Model_mock
        model.run_action_on_leader("app", "backup", action_params={"backup_dir": "/dev/null"})
    ...
    def test_run_action_on_units(self):
        ...
        units = {"app/1": self.unit1, "app/2": self.unit2}
        ...
        model.run_action_on_units(
            ["app/1", "app/2"], "backup", action_params={"backup_dir": "/dev/null"}
        )
        self.unit1.run_action.assert_called_once_with("backup", backup_dir="/dev/null")
        self.unit2.run_action.assert_called_once_with("backup", backup_dir="/dev/null")
    ...
    def test_run_action_on_units_timeout(self):
        ...
        with self.assertRaises(AsyncTimeoutError):
            model.run_action_on_units(
                ["app/2"], "backup", action_params={"backup_dir": "/dev/null"}, timeout=0.1
            )
    ...
    def _application_states_setup(self, setup, units_idle=True):
        ...
        p_mock_ws = mock.PropertyMock(return_value=setup["workload-status"])
        wsmsg = setup.get("workload-status-message-prefix", setup.get("workload-status-message"))
        ...
        self.patch("asyncio.sleep", name="mock_asyncio_sleep", new=mock.AsyncMock())
        self.patch_object(model.logging, "info", name="mock_logging_info")
        self.patch_object(model.logging, "warning", name="mock_logging_warning")

    def test_units_with_wl_status_state(self):
        self._application_states_setup(
            {"workload-status": "active", "workload-status-message": "Unit is ready"}
        )
        with mock.patch.object(zaza, "RUN_LIBJUJU_IN_THREAD", new=False):
    ...
    def test_units_with_wl_status_state_no_match(self):
        self._application_states_setup(
            {"workload-status": "blocked", "workload-status-message": "Unit is ready"}
        )
        with mock.patch.object(zaza, "RUN_LIBJUJU_IN_THREAD", new=False):
            units = model.units_with_wl_status_state(self.Model_mock, "active")
    ...
    def test_machines_in_state(self):
        self.assertEqual(model.machines_in_state(self.Model_mock, ["provisioning error"]), [])
        machine_error_mock = mock.MagicMock(status="provisioning error")
        self.unit1.machine = machine_error_mock
        self.assertEqual(
            model.machines_in_state(self.Model_mock, ["provisioning error"]), [machine_error_mock]
        )
    ...
    def test_check_unit_workload_status(self):
        ...
        with mock.patch.object(zaza, "RUN_LIBJUJU_IN_THREAD", new=False):
            self.assertTrue(
                model.check_unit_workload_status(self.Model_mock, self.unit1, ["active"])
            )
        ...
        with mock.patch.object(zaza, "RUN_LIBJUJU_IN_THREAD", new=False):
            self.assertTrue(
                model.check_unit_workload_status(
                    self.Model_mock, self.unit1, ["active", "blocked"]
                )
            )
    ...
    def test_check_unit_workload_status_message_message_not_found(self):
        self.patch_object(model, "check_model_for_hard_errors")
        self._application_states_setup(
            {"workload-status": "blocked", "workload-status-message": "Something else"}
        )
        with mock.patch.object(zaza, "RUN_LIBJUJU_IN_THREAD", new=False):
            self.assertFalse(
                model.check_unit_workload_status_message(
    ...
    def test_check_unit_workload_status_message_prefix(self):
        ...
        with mock.patch.object(zaza, "RUN_LIBJUJU_IN_THREAD", new=False):
            self.assertTrue(
                model.check_unit_workload_status_message(
                    self.Model_mock, self.unit1, prefixes=["Readyish", "Unit is ready"]
                )
            )
    ...
    def test_check_unit_workload_status_message_prefix_no_match(self):
        self.patch_object(model, "check_model_for_hard_errors")
        self._application_states_setup(
            {"workload-status": "blocked", "workload-status-message": "On my holidays"}
        )
        with mock.patch.object(zaza, "RUN_LIBJUJU_IN_THREAD", new=False):
            self.assertFalse(
                model.check_unit_workload_status_message(
    ...
    def test_check_unit_workload_status_message_regex(self):
        ...
        with mock.patch.object(zaza, "RUN_LIBJUJU_IN_THREAD", new=False):
            self.assertTrue(
                model.check_unit_workload_status_message(
                    self.Model_mock, self.unit1, regex=r"my\sholiday.$"
                )
            )

    def test_is_unit_errored_from_install_hook(self):
        self._application_states_setup(
            {"workload-status": "error", "workload-status-message": 'hook failed: "install"'}
        )
        with mock.patch.object(zaza, "RUN_LIBJUJU_IN_THREAD", new=False):
            self.assertTrue(model.is_unit_errored_from_install_hook(self.unit1))
    ...
    def test_is_unit_errored_from_install_hook_not_install_hook(self):
        ...
        with mock.patch.object(zaza, "RUN_LIBJUJU_IN_THREAD", new=False):
            self.assertFalse(model.is_unit_errored_from_install_hook(self.unit1))

    def test_is_unit_errored_from_install_hook_not_error(self):
        self._application_states_setup(
            {"workload-status": "blocked", "workload-status-message": "waiting on the sunset"}
        )
        with mock.patch.object(zaza, "RUN_LIBJUJU_IN_THREAD", new=False):
            self.assertFalse(model.is_unit_errored_from_install_hook(self.unit1))
    ...
    def test_wait_for_application_states_errored_unit(self):
        self._application_states_setup(
            {"workload-status": "error", "workload-status-message": "Unit is ready"}
        )
        with mock.patch.object(zaza, "RUN_LIBJUJU_IN_THREAD", new=False):
            with self.assertRaises(model.UnitError):
                model.wait_for_application_states("modelname", timeout=1)
    ...
    def test_wait_for_application_states_errored_unit_ignore(self):
        ...
        with mock.patch.object(zaza, "RUN_LIBJUJU_IN_THREAD", new=False):
            ...
            with self.assertRaises(model.ModelTimeout):
                model.wait_for_application_states("modelname", timeout=1, ignore_hard_errors=True)
                self.assertFalse(self.system_ready)
    ...
    def test_wait_for_application_states_retries_no_success(self):
        ...
        with mock.patch.object(zaza, "RUN_LIBJUJU_IN_THREAD", new=False):
            with self.assertRaises(model.UnitError):
                model.wait_for_application_states("modelname", timeout=1, max_resolve_count=3)
                self.assertFalse(self.system_ready)
    ...
    def test_wait_for_application_states_retries_with_success(self):
        ...
        with mock.patch.object(zaza, "RUN_LIBJUJU_IN_THREAD", new=False):
            model.wait_for_application_states("modelname", timeout=500, max_resolve_count=3)
        self.assertEquals(self.async_resolve_units.call_count, 2)
        self.async_block_until_unit_wl_status.assert_has_calls(
            [
                mock.call("app/2", "error", "modelname", negate_match=True, timeout=60),
                mock.call("app/2", "error", "modelname", negate_match=True, timeout=60),
            ]
        )
    ...
    def test_wait_for_application_states_blocked_ok(self):
        ...
        with mock.patch.object(zaza, "RUN_LIBJUJU_IN_THREAD", new=False):
            model.wait_for_application_states(
                "modelname", states={"app": {"workload-status": "blocked"}}, timeout=1
            )
        self.assertTrue(self.system_ready)

    def test_wait_for_application_states_bespoke_msg(self):
        self._application_states_setup(
            {"workload-status": "active", "workload-status-message": "Sure, I could do something"}
        )
        with mock.patch.object(zaza, "RUN_LIBJUJU_IN_THREAD", new=False):
            model.wait_for_application_states(
                "modelname",
                states={"app": {"workload-status-message": "Sure, I could do something"}},
                timeout=1,
            )
        self.assertTrue(self.system_ready)
    ...
    def test_wait_for_application_states_bespoke_msg_blocked_ok(self):
        self._application_states_setup(
            {"workload-status": "blocked", "workload-status-message": "Sure, I could do something"}
        )
        with mock.patch.object(zaza, "RUN_LIBJUJU_IN_THREAD", new=False):
            model.wait_for_application_states(
                "modelname",
    ...
    def test_wait_for_application_states_idle_timeout(self):
        ...
        self.assertEqual(timeout.exception.args[0], "Work state not achieved within timeout.")
    ...
    def test_wait_for_application_states_timeout(self):
        ...
        self.assertIn(mock.call("Applications left: %s", "app"), self.mock_logging_info.mock_calls)
    ...
    def test_file_contents_success(self):
        self.action.data = {"results": {"Code": "0", "Stderr": "", "Stdout": "somestring"}}

        self.patch_object(model, "Model")
        self.Model.return_value = self.Model_mock
        self.patch_object(model, "get_juju_model", return_value="mname")
        contents = model.file_contents("app/2", "/tmp/src/myfile.txt", timeout=0.1)
        self.unit1.run.assert_called_once_with("cat /tmp/src/myfile.txt", timeout=0.1)
        self.assertEqual("somestring", contents)

    def test_file_contents_fault(self):
        self.action.data = {"results": {"Code": "0", "Stderr": "fault", "Stdout": ""}}
    ...
    def test_block_until_file_has_contents(self):
        ...
        self.Model.return_value = self.Model_mock
        self.patch_object(model, "get_juju_model", return_value="mname")
        self.patch("builtins.open", new_callable=mock.mock_open(), name="_open")
        ...
        model.block_until_file_has_contents(
            "app", "/tmp/src/myfile.txt", "somestring", timeout=0.1
        )
        self.unit1.run.assert_called_once_with("cat /tmp/src/myfile.txt")
        self.unit2.run.assert_called_once_with("cat /tmp/src/myfile.txt")

    def test_block_until_file_has_no_contents(self):
        self.action.data = {"results": {"Code": "0", "Stderr": ""}}
        ...
        self._open.return_value = _fileobj
        model.block_until_file_has_contents("app", "/tmp/src/myfile.txt", "", timeout=0.1)
        self.unit1.run.assert_called_once_with("cat /tmp/src/myfile.txt")
        self.unit2.run.assert_called_once_with("cat /tmp/src/myfile.txt")
    ...
    def test_block_until_file_missing(self):
        ...
        model.block_until_file_missing("app", "/tmp/src/myfile.txt", timeout=0.1)
        self.unit1.run.assert_called_once_with('test -e "/tmp/src/myfile.txt"; echo $?')
    ...
    def test_block_until_file_matches_re(self):
        ...
        self.Model.return_value = self.Model_mock
        self.patch_object(model, "get_juju_model", return_value="mname")
        model.block_until_file_matches_re("app", "/tmp/src/myfile.txt", "s.*string", timeout=0.1)
        self.unit1.run.assert_called_once_with("cat /tmp/src/myfile.txt")
        self.unit2.run.assert_called_once_with("cat /tmp/src/myfile.txt")
    ...
    def test_block_until_unit_count(self):
        ...
        self.juju_status.applications[self.application]["units"] = ["app/1", "app/2"]
    ...
    def test_block_until_service_status_check_running(self):
        ...
        model.block_until_service_status("app/2", ["test_svc"], "running")

    def test_block_until_service_status_check_running_with_pgrep(self):
        self.patch_object(model, "get_juju_model", return_value="mname")
        self.block_until_service_status_base({"Stdout": "152 409 54"})
        model.block_until_service_status("app/2", ["test_svc"], "running", pgrep_full=True)
        self.async_run_on_unit.assert_called_once_with(
            "app/2", "pgrep -f 'test_svc'", model_name=None, timeout=2700
        )
    ...
    def test_block_until_service_status_check_stopped(self):
        self.patch_object(model, "get_juju_model", return_value="mname")
        self.block_until_service_status_base({"Stdout": ""})
        model.block_until_service_status("app/2", ["test_svc"], "stopped")
    ...
    def test_get_unit_time(self):
        async def _run_on_unit(unit_name, command, model_name=None, timeout=None):
            return {"Stdout": "1524409654"}

        self.patch_object(model, "async_run_on_unit")
        self.async_run_on_unit.side_effect = _run_on_unit
        self.assertEqual(model.get_unit_time("app/2"), 1524409654)
        self.async_run_on_unit.assert_called_once_with(
            unit_name="app/2", command="date +'%s'", model_name=None, timeout=None
        )

    def test_get_systemd_service_active_time(self):
        async def _run_on_unit(unit_name, command, model_name=None, timeout=None):
            return {"Stdout": "ActiveEnterTimestamp=Fri 2022-01-14 13:32:24 UTC"}
        ...
        self.async_run_on_unit.assert_called_once_with(
            unit_name="app/2", command=cmd, model_name=None, timeout=None
        )
    ...
    def test_get_unit_service_start_time(self):
        ...
        self.patch_object(model, "async_run_on_unit")
        self.async_run_on_unit.side_effect = _run_on_unit
        self.assertEqual(model.get_unit_service_start_time("app/2", "mysvc1"), 1524409654)
        cmd = (
            r"pidof -x 'mysvc1'| tr -d '\n' | "
    ...
    def test_get_unit_service_start_time_with_pgrep(self):
        ...
        self.async_run_on_unit.side_effect = _run_on_unit
        self.assertEqual(
            model.get_unit_service_start_time("app/2", "mysvc1", pgrep_full=True), 1524409654
        )
    ...
    def block_until_oslo_config_entries_match_base(self, file_contents, expected_contents):
        self.action.data = {"results": {"Code": "0", "Stderr": "", "Stdout": file_contents}}
        self.patch_object(model, "Model")
        self.patch_object(model, "get_juju_model", return_value="mname")
        self.Model.return_value = self.Model_mock
        model.block_until_oslo_config_entries_match(
            "app", "/tmp/src/myfile.txt", expected_contents, timeout=0.1
        )
...
"""
        ...
        self.block_until_oslo_config_entries_match_base(file_contents, expected_contents)
        self.unit1.run.assert_called_once_with("cat /tmp/src/myfile.txt")
        self.unit2.run.assert_called_once_with("cat /tmp/src/myfile.txt")
...
"""
    ...
    def block_until_services_restarted_base(self, gu_return=None, gu_raise_exception=False):
        async def _block_until(f, timeout=None):
            rc = await f()
            if not rc:
                raise AsyncTimeoutError
        ...
        async def _async_get_unit_service_start_time(
            unit, svc, timeout=None, model_name=None, pgrep_full=False
        ):
        ...
        self.async_get_unit_service_start_time.side_effect = _async_get_unit_service_start_time
        self.patch_object(model, "Model")
        self.Model.return_value = self.Model_mock
    ...
    def test_block_until_services_restarted_with_pgrep(self):
        self.block_until_services_restarted_base(gu_return=10)
        model.block_until_services_restarted("app", 8, ["svc1", "svc2"], pgrep_full=True)
        self.async_get_unit_service_start_time.assert_has_calls(
            [
                mock.call("app/2", "svc1", model_name=None, pgrep_full=True, timeout=2700),
                mock.call("app/2", "svc2", model_name=None, pgrep_full=True, timeout=2700),
                mock.call("app/4", "svc1", model_name=None, pgrep_full=True, timeout=2700),
                mock.call("app/4", "svc2", model_name=None, pgrep_full=True, timeout=2700),
            ]
        )
    ...
    def test_block_until_services_restarted_fail(self):
        self.block_until_services_restarted_base(gu_return=10)
        with self.assertRaises(AsyncTimeoutError):
            model.block_until_services_restarted("app", 12, ["svc1", "svc2"])
    ...
    def test_block_until_unit_wl_status(self):
        ...
        model.block_until_unit_wl_status("subordinate_application/1", "active", timeout=0.1)
    ...
    def test_block_until_unit_wl_status_fail(self):
        ...
        (
            self.juju_status.applications[self.application]["units"][self.unit]["workload-status"][
                "status"
            ]
        ) = "blocked"
        (
            self.juju_status.applications[self.application]["units"][self.unit]["subordinates"][
                self.subordinate_unit
    ...
    def test_block_until_unit_wl_status_inverse(self):
        ...
        self.patch_object(model, "async_block_until")
        self.async_block_until.side_effect = _block_until
        model.block_until_unit_wl_status("app/1", "unknown", negate_match=True, timeout=0.1)
        model.block_until_unit_wl_status(
            "subordinate_application/1", "unknown", negate_match=True, timeout=0.1
        )
    ...
    def test_block_until_wl_status_info_starts_with(self):
        ...
        self.patch_object(model, "get_unit_from_name")
        self.patch_object(model, "async_get_status")
        self.juju_status.applications["app"]["units"]["app/1"]["workload-status"][
            "info"
        ] = "match-me if you want"
        self.juju_status.applications["app"]["units"]["app/2"]["workload-status"][
            "info"
        ] = "match-me if you want"
        self.async_get_status.side_effect = _get_status
        self.patch_object(model, "async_block_until")
        self.async_block_until.side_effect = _block_until
        model.block_until_wl_status_info_starts_with("app", "match-me")
    ...
    def test_block_until_wl_status_info_starts_with_negative(self):
        ...
        self.patch_object(model, "async_block_until")
        self.async_block_until.side_effect = _block_until
        model.block_until_wl_status_info_starts_with("app", "dont-match-me", negate_match=True)
    ...
    def test_block_until_unit_wl_message_match(self):
        ...
        self.patch_object(model, "async_block_until")
        self.async_block_until.side_effect = _block_until
        model.block_until_unit_wl_message_match("app/1", "(m|p)atch-me.*")
    ...
    def test_block_until_unit_wl_message_match_negative(self):
        ...
        self.patch_object(model, "async_block_until")
        self.async_block_until.side_effect = _block_until
        model.block_until_unit_wl_message_match("app/1", "(w|p)atch-me.*", negate_match=True)
    ...
    def test_resolve_units(self):
        ...
        self.check_output.assert_called_once_with(["juju", "resolved", "-m", "testmodel", "app/2"])
    ...
    def test_upgrade_charm(self):
        ...
        model.upgrade_charm("myapp", switch="cs:~me/new-charm-45")
    ...
    def test_get_latest_charm_url(self):
        ...
        self.assertEqual(model.get_latest_charm_url("cs:something"), "cs:something-23")

    def test_prepare_series_upgrade(self):
        self.patch_object(model, "subprocess")
        self.patch_object(model, "get_juju_model", return_value=self.model_name)
        _machine_num = "1"
        _to_series = "bionic"
    ...
    def test_complete_series_upgrade(self):
        ...
        self.subprocess.check_call.assert_called_once_with(
            ["juju", "upgrade-series", "-m", self.model_name, _machine_num, "complete"]
        )
    ...
    def test_set_series(self):
        ...
        self.subprocess.check_call.assert_called_once_with(
            ["juju", "set-series", "-m", self.model_name, _application, _to_series]
        )
    ...
    def test_add_storage(self):
        ...
        self.subprocess.check_output.assert_called_once_with(
            ["juju", "add-storage", "ceph-osd/0", "-m", self.model_name, "label=pool,101GB"],
            stderr=mock.ANY,
        )
    ...
    def test_detach_storage(self):
        ...
        self.subprocess.check_call.assert_called_once_with(
            ["juju", "detach-storage", "-m", self.model_name, "storage", "--force"]
        )
    ...
    def test_remove_storage(self):
        ...
        self.subprocess.check_call.assert_called_once_with(
            ["juju", "remove-storage", "storage", "-m", self.model_name, "--force", "--no-destroy"]
        )
...
class AsyncModelTests(aiounittest.AsyncTestCase):
    ...
    async def test_run_on_machine(self):
        with mock.patch.object(model.generic_utils, "check_output") as check_output:
            await model.async_run_on_machine("1", "test")
        check_output.assert_called_once_with(["juju", "run", "--machine=1", "--", "test"])
    ...
    async def test_async_get_agent_status(self):
        ...
        with mock.patch.object(model, "async_get_status", return_value=model_mock):
    ...
    async def test_async_get_principle_sub_map(self):
        model_mock = mock.MagicMock()
        model_mock.applications = {"app": {"units": FAKE_STATUS["units"]}}
        with mock.patch.object(model, "async_get_status", return_value=model_mock):
    ...
    async def test_async_get_principle_unit(self):
        model_mock = mock.MagicMock()
        model_mock.applications = {"app": {"units": FAKE_STATUS["units"]}, "dsql": {"units": {}}}
        with mock.patch.object(model, "async_get_status", return_value=model_mock):
    ...
    async def test_async_get_cloud_data(self):
        with mock.patch.object(model.juju.client.jujudata, "FileJujuData") as juju_data:
            with mock.patch.object(model, "get_model"):
                juju_data().load_credential.return_value = ("fake-cred-name", "fake-cred")
