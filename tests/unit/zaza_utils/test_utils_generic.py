...
import cou.zaza_utils.exceptions as zaza_exceptions
...
class TestGenericUtils(ut_utils.BaseTestCase):
    ...
    def test_dict_to_yaml(self):
        ...
        _str_data = "key: value\n"
        self.assertEqual(generic_utils.dict_to_yaml(_dict_data), _str_data)
    ...
    def test_get_network_config(self):
        ...
        self.assertEqual(
            generic_utils.get_network_config(net_topology, ignore_env_vars=True),
            _data[net_topology],
        )
        ...
        _more_data = {"network": "NEW", "other": "DATA"}
        self.get_undercloud_env_vars.return_value = _more_data
        _data[net_topology].update(_more_data)
        self.assertEqual(generic_utils.get_network_config(net_topology), _data[net_topology])
    ...
    def test_get_undercloud_env_vars(self):
        ...
        self.assertEqual(generic_utils.get_undercloud_env_vars(), _expected_result)

        # Overriding configure.network named variables
        _override = {
            "start_floating_ip": "10.100.50.0",
    ...
    def test_get_yaml_config(self):
        ...
        self.assertEqual(generic_utils.get_yaml_config(_filename), _yaml_dict)
    ...
    def test_wrap_do_release_upgrade(self):
        ...
        _scp_calls = []
        _run_calls = [mock.call(_unit, _workaround_script)]
    ...
    def test_juju_reboot(self):
        ...
        self.subprocess.check_call.assert_called_once_with(
            ["juju", "ssh", _unit, f'sudo juju-run -u {_unit} "juju-reboot --now"']
        )
    ...
    def test_run_via_ssh(self):
        ...
        self.subprocess.check_call.assert_called_once_with(["juju", "ssh", _unit, "sudo " + _cmd])
    ...
    def test_set_origin(self):
        ...
        self.set_application_config.assert_called_once_with(_application, {_origin: _pocket})
    ...
    def test_series_upgrade(self):
        ...
        self.prepare_series_upgrade.assert_called_once_with(_machine_num, to_series=_to_series)
    ...
    def test_series_upgrade_application_pause_peers_and_subordinates(self):
        ...
        _run_action_calls = [
            mock.call("{}-hacluster/1".format(_application), "pause", action_params={}),
            mock.call("{}/1".format(_application), "pause", action_params={}),
            mock.call("{}-hacluster/2".format(_application), "pause", action_params={}),
        ...
        for machine_num in ("0", "1", "2"):
            _series_upgrade_calls.append(
                mock.call(
                    "{}/{}".format(_application, machine_num),
                    machine_num,
                    origin=_origin,
                    from_series=_from_series,
                    to_series=_to_series,
    ...
    def test_get_process_id_list(self):
        self.patch(
            "cou.zaza_utils.generic.model.run_on_unit", new_callable=mock.MagicMock(), name="_run"
        )

        # Return code is OK and STDOUT contains output
        returns_ok = {"Code": 0, "Stdout": "1 2", "Stderr": ""}
        self._run.return_value = returns_ok
        p_id_list = generic_utils.get_process_id_list("ceph-osd/0", "ceph-osd", False)
        expected = ["1", "2"]
        cmd = 'pidof -x "ceph-osd" || exit 0 && exit 1'
        self.assertEqual(p_id_list, expected)
        self._run.assert_called_once_with(unit_name="ceph-osd/0", command=cmd)

        # Return code is not OK
        returns_nok = {"Code": 1, "Stdout": "", "Stderr": "Something went wrong"}
    ...
    def test_get_process_id_list_with_pgrep(self):
        ...
        p_id_list = generic_utils.get_process_id_list(
            "ceph-osd/0", "ceph-osd", False, pgrep_full=True
        )
        expected = ["1", "2"]
    ...
    def test_get_unit_process_ids(self):
        self.patch(
            "cou.zaza_utils.generic.get_process_id_list",
        ...
        unit_processes = {"ceph-osd/0": {"ceph-osd": 2}, "unit/0": {"pr1": 2, "pr2": 2}}
        expected = {
            "ceph-osd/0": {"ceph-osd": ["1", "2"]},
            "unit/0": {"pr1": ["1", "2"], "pr2": ["1", "2"]},
        }
        result = generic_utils.get_unit_process_ids(unit_processes)
        self.assertEqual(result, expected)

    def test_validate_unit_process_ids(self):
        expected = {"ceph-osd/0": {"ceph-osd": 2}, "unit/0": {"pr1": 2, "pr2": [1, 2]}}
    ...
    def test_check_call(self):
        ...
        self.patch_object(generic_utils, "check_output", side_effect=async_check_output)
        check_call = sync_wrapper(generic_utils.check_call)
        check_call("a command")
        self.check_output.assert_called_once_with("a command", log_stdout=True, log_stderr=True)
        self.check_output.reset_mock()
        check_call("b command", log_stdout=False)
        self.check_output.assert_called_once_with("b command", log_stdout=False, log_stderr=True)
        self.check_output.reset_mock()
        check_call("c command", log_stderr=False)
        self.check_output.assert_called_once_with("c command", log_stdout=True, log_stderr=False)
    ...
    def test_check_output(self):
        ...
        self.patch_object(
            generic_utils.asyncio,
            "create_subprocess_exec",
            side_effect=mock_create_subprocess_exec,
        )
        check_call = sync_wrapper(generic_utils.check_output)
        expected = {
